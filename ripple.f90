module variaveis
    ...
    integer, parameter:: ni = 5000, ns = 2, nc = 3000, &
&                        corte = 1, relax = 0
...
&                                beta = betadamp * damping
    ...
    doubleprecision, parameter:: rx = 2.d0, ry = 2.d0
    ...
    doubleprecision:: fcx, fcy, aux, hx, hy, wx, vrx, vry, hxs, hys
    doubleprecision, parameter:: ax = lx / 2.d0, ay = ly / 2.d0
...
module gera_function
    ...
    doubleprecision, dimension(0: n):: y_bessel
    doubleprecision, parameter:: aw = 0.d0, ab = 0.5d0
    doubleprecision::bb, bw, hb, hw_
...
subroutine skyr_ripple(x0, y0, rx, ry, u, vry, vrx, lx, ly, ay, ax)
    implicit none
    doubleprecision:: x0, y0, rx, ry, u, vry, vrx, aux, lx, ly, ay, ax
    aux = ((x0 - ax) / rx) ** 2.d0 + ((y0 - ay) / ry) ** 2.d0
    vrx = 2.d0 * u * (x0 - ax) * dsin(aux) / (rx * rx)
    vry = 2.d0 * u * (y0 - ay) * dsin(aux) / (ry * ry)
end subroutine
...
subroutine forcas(desx, desy)
    ...
    do i___ = -1, 1
        ...
        enddo
    enddo
    call skyr_ripple(x0(i_s) + desx, y0(i_s) + desy, rx, ry, u, vry, vrx, lx, ly, ax, ay)
    call random_number(na)
    x_temp = (2.d0 * na - 1.d0) * dsqrt(temp)
    call random_number(na)
    y_temp = (2.d0 * na - 1.d0) * dsqrt(temp)
    
    fcx = fc * dsin(2.d0 * pi * omega * dble(t))
    fcy = fc * dcos(2.d0 * pi * omega * dble(t)) * 0.d0
    p1 = ssx + vrx + x_temp + fcx
    p2 = ssy + vry + y_temp + fcy
